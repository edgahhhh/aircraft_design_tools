...
class ActuatingDiskParams(NamedTuple):
    """ actuation disk params struct of named tuple type """
    diameter:float


class ActuationDisk():
    """ actuating disk model """
    def __init__(self, params:ActuatingDiskParams, atmospheric_properties):
        """ create an actuating disk model
        s
        Arguments:
            params: ActuatingDiskParams
                struct of parameters for the actuating disk model
            TODO atmospheric_properties: 
        """
        assert isinstance(params, ActuatingDiskParams), "params must be of ActuatingDiskParams"


class ActuatingDisk():
    """ actuating disk model """
    def __init__(self, diam, alt=400.0):
        """
