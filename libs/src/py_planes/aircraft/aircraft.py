...
from py_planes.propeller import Propeller, PropellerAnalysis, rpm_to_omega, omega_to_rpm
# from propeller_analysis import PropellerAnalysis
#pylint: disable=redefined-outer-name
#pylint: disable=line-too-long

class AircraftInfo(NamedTuple):
    """ Aircraft Info named tuple class """

class MissionTwoInfo(NamedTuple):
    """ mission two info named tuple class """

class MissionThreeInfo(NamedTuple):
    """ mission three info named tuple class """
...
# TODO: change the variables in these classes to not include power and energy
# This is a bit of a chore, and requires changes anywhere the classes are called...
...
class NewDynamics():
    """ Dynamics object for just an aircraft... """
    def __init__(self,
                 ref_area,
                 aspect_ratio,
                 polar_info,
    ...
    def get_coefficients(self,airspeed, aoa_plane, load_factor):
        ...
        cd=cd_fus+cd_wing_tail+cdi_new-cdi_old+cd_gear
        cd=cd*(1+self._crud)
        return cl_wing_tail, cd

    def get_drag_coefficient_legacy(self,airspeed,weight,load_factor):
...
class Aircraft:
    ...
    def best_roc(self, mass, propeller:Propeller, rpm, banner=False,vmin=5., vmax=30., iter=1000):
        """ get the best rate of climb airspeed for the aircraft
        
        Returns:
            vy: airspeed for best rate of climb m/s
            vv: climb rate at vy, m/s
        """
        airspeeds=np.linspace(vmin, vmax, iter)
        n=rpm_to_omega(rpm)    # assume max throttle here
        ROC = []
        for v in airspeeds:
            cl, cd = self.dynamics.get_drag_coefficient_legacy(v, (mass*9.81), 1, banner=banner, stowed=True)
            _, drag = self.dynamics.calculate_dynamics(v, cl, cd)
            power_avail=propeller.calculate_thrust(v, n) * v
            power_req = drag * v
            power_exc = power_avail - power_req
            roc = power_exc / mass / 9.81
            ROC.append(roc)
        vy = airspeeds[np.argmax(ROC)]
        return vy, max(ROC)
    ...
    def take_off_performance(
            self,mass,propeller:Propeller, motor_eff=0.9,
            sg=17.5,banner=False,stowed=False
                             ) -> Tuple[
                                 float,
                                 Performance,
                                 Power,
        ...
        Returns:
            ...
            power (Power)
            energy (Energy)
            propeller_performance (PropellerAnalysis)
        """
        ...
        # run power analysis for propeller
        power_aero=drag*v
        prop_analysis=propeller.analysis(v, thrust)
        ...
        power_batt=power_motor/0.9
        power=Power(power_aero=power_aero,
                    power_propeller=power_propeller,
                    power_motor=power_motor,
                    power_battery=power_batt)
        energy_batt=power_batt*t_tof/3600
        energy=Energy(energy_battery=energy_batt,
                      energy_aero=0.0,
                      energy_motor=0.0,
                      energy_propeller=0.0)
        return t_tof, performance, power, energy, prop_analysis
    ...
    def climb_performance(self,mass, propeller:Propeller, motor_eff=0.9,
        ...
        climb_rpm = 4*3.75 * 340    # TODO: clean out this hardcoded rpm for the climb, may have issues for the banner mission...
        v_climb, vv =self.best_roc(mass, propeller, climb_rpm, banner=True)
        ...
        # Propeller stuff...
        thrust=propeller.calculate_thrust(v_climb, rpm_to_omega(climb_rpm))
        power_aero=propeller.calculate_power(v_climb, rpm_to_omega(climb_rpm))

        # Get powers for aircraft
        # thrust=drag
        # power_aero=thrust*v_climb
        prop_analysis=propeller.analysis(v_climb, thrust)   # TODO: this is super redundant...
        if prop_analysis is not None:
            power_propeller=prop_analysis.power
        else:
            warnings.warn(
        ...
        power_batt=power_motor/0.9
        climb_power=Power(power_aero=power_aero,
                          power_battery=power_batt, 
                          power_motor=power_motor,
                          power_propeller=power_propeller)
        # TODO: #11 solve for time needed to climb (this is small so ignore for now)
        # scale power list by time for energy
        t_climb=self.calculate_time_to_climb(climb_rate=vv, cruise_alt=50)
        energy_batt=power_batt*t_climb/3600

        climb_energy=Energy(energy_battery=energy_batt,
                            energy_aero=0.0,
                            energy_motor=0.0,
                            energy_propeller=0.0)
        
        print(f' \n\n\nclimb rate: {vv} m/s \nt climb: {t_climb} s\nvy: {v_climb} m/s \nenergy_climb: {energy_batt} Whr \nclimb power: {power_batt} W at batt')
        print(f'thrust for climb: {thrust} N \nrpm: {climb_rpm}')
        return t_climb, climb_performance, climb_power, climb_energy, prop_analysis
    ...
    def lap_performance(self,
        ...
        Returns:
            energy_total        (float)
            t_total             (float)
            cruise_performance  (Performance)
            turn_performance    (Performance)
            propeller_cruise    (PropellerAnalysis)
            propeller_turn      (PropellerAnalysis)
            cruise_power        (Power)
            turn_power          (Power)
        """
        ...
        lift_cruise, drag_cruise = self.dynamics.calculate_dynamics(
        ...
        lift_turn,drag_turn=self.dynamics.calculate_dynamics(
            airspeed=v_cruise,cl=cl_turn,cd=cd_turn
        )
        # solve for power for cruising
        power_cruise_aero=drag_cruise*v_cruise
        prop_perf_cruise=propeller.analysis(airspeed=v_cruise, drag=drag_cruise)
        if prop_perf_cruise is None:
            warnings.warn('propeller did not converge for cruise, defaulting to 40% prop eta', 
                          UserWarning, stacklevel=2)
            power_cruise_prop=power_turn_aero/0.4
        else:
            power_cruise_prop=prop_perf_cruise.power
        power_cruise_motor=power_cruise_prop/motor_eff
        power_cruise_batt=power_cruise_motor/0.9

        # solve for power for turning
        power_turn_aero=drag_turn*v_cruise
        prop_perf_turn = propeller.analysis(airspeed=v_cruise, drag=drag_turn)
        if prop_perf_turn is None:
            warnings.warn('propeller did not converge for turning, defaulting to 40% prop eta', 
                          UserWarning, stacklevel=2)
            power_turn_prop=power_turn_aero/0.4
        else:
            power_turn_prop=prop_perf_turn.power
        power_turn_motor=power_turn_prop/motor_eff
        power_turn_batt=power_turn_motor/0.9
        ...
        print(f' \nlap time: {t_lap_ea} seconds \ntotal time for mission: {t_total} seconds \n')
        ...
        cruise_performance=Performance(cl=cl_cruise, 
                                       cd=cd_cruise,
                                       lift=lift_cruise,
                                       drag=drag_cruise,
                                       airspeed=v_cruise)
        turn_performance=Performance(cl=cl_turn,
                                     cd=cd_turn,
                                     lift=lift_turn,
                                     drag=drag_turn,
                                     airspeed=v_cruise)
        cruise_power=Power(power_aero=power_cruise_aero,
                           power_motor=power_cruise_motor,
                           power_propeller=power_cruise_prop,
                           power_battery=power_cruise_batt)
        turn_power=Power(power_aero=power_turn_aero,
                         power_battery=power_turn_batt,
                         power_motor=power_turn_motor,
                         power_propeller=power_turn_prop)
        energy_laps=Energy(energy_battery=energy_total,
                           energy_aero=0.0,
                           energy_motor=0.0,
                           energy_propeller=0.0)
        return (energy_laps, t_total, cruise_performance, turn_performance,
                prop_perf_cruise, prop_perf_turn, cruise_power, turn_power)
