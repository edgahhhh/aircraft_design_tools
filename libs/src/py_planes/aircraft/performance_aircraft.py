...
class PerformanceAircraft(Aircraft):
    ...
    def _mission_two_performance(self):
        ...
        # energy_tof_float b/c its not an Energy object
        t_tof, perf_tof, power_tof, energy_tof, prop_tof=self.take_off_performance(
            mass=self.m2_gross_mass,
            propeller=self.m2_prop)
        # calculate climb performance
        t_climb, perf_climb, power_climb, energy_climb, prop_climb =self.climb_performance(
            mass=self.m2_gross_mass,
            propeller=self.m2_prop)
        # calculate lap performance
        # I call it energy_laps_float since its not an Energy object
        (energy_laps, t_laps, perf_cruise, perf_turn, prop_cruise,
         prop_turn, power_cruise, power_turn) = self.lap_performance(
             laps=5,
             mass=self.m2_gross_mass,
        ...
        # save things to class
        self.m2_t_tof=t_tof
        self.m2_perf_tof=perf_tof
        self.m2_power_tof=power_tof
        self.m2_energy_tof=energy_tof
        self.m2_prop_perf_tof=prop_tof

        self.m2_t_climb=t_climb
        self.m2_perf_climb=perf_climb
        self.m2_power_climb=power_climb
        self.m2_energy_climb=energy_climb
        self.m2_prop_perf_climb=prop_climb

        self.m2_energy_laps=energy_laps
        self.m2_t_laps=t_laps
        self.m2_perf_cruise=perf_cruise
        self.m2_perf_turn=perf_turn
        self.m2_prop_perf_cruise=prop_cruise
        self.m2_prop_perf_turn=prop_turn
        self.m2_power_cruise=power_cruise
        self.m2_power_turn=power_turn
        
        # find total energy for mission two and the total time for mission two
        energy_total = (
            energy_laps.energy_battery+energy_tof.energy_battery+energy_climb.energy_battery
            ) / 0.75
        time_total = t_laps + t_tof + t_climb
    ...
    def _mission_three_performance(self):
        ...
        # calculate the takeoff performance
        t_tof, perf_tof, power_tof, energy_tof, prop_tof = self.take_off_performance(
            mass=self.m3_gross_mass,
        ...
        # calculate the laps performance
        (energy_laps, t_laps, perf_cruise, perf_turn, prop_cruise,
         prop_turn, power_cruise, power_turn) = self.lap_performance(
             laps=4,
             mass=self.m3_gross_mass,
        ...
        # save things to class
        self.m3_t_tof=t_tof
        self.m3_perf_tof=perf_tof
        self.m3_power_tof=power_tof
        self.m3_energy_tof=energy_tof
        self.m3_prop_perf_tof=prop_tof

        self.m3_t_climb=t_climb
        self.m3_perf_climb=perf_climb
        self.m3_power_climb=power_climb
        self.m3_energy_climb=energy_climb
        self.m3_prop_perf_climb=prop_climb

        self.m3_energy_laps=energy_laps
        self.m3_t_laps=t_laps
        self.m3_perf_cruise=perf_cruise
        self.m3_perf_turn=perf_turn
        self.m3_prop_perf_cruise=prop_cruise
        self.m3_prop_perf_turn=prop_turn
        self.m3_power_cruise=power_cruise
        self.m3_power_turn=power_turn
        

        alpha_m3 = self.dynamics._aoa_from_cl_interp(
            perf_cruise.cl)
        energy_total= (
            energy_laps.energy_battery + energy_climb.energy_battery + energy_tof.energy_battery
                       ) / 0.8
        time_total = t_laps + t_tof + t_climb
